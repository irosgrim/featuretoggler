...
const Main: React.FC<MainProps> = ({ activeProject, currentView, onCurrentView, onProjectDelete, children }): React.ReactElement => {
	const { features, newFeature } = useStore((state) => ({
	...
	const filteredFeaturesByProject = features
		? features.filter((f) => {
				if (activeProject) {
	...
	const createNewFeatureToggle = async (featureToggle: { name: string; value: string; description: string; enabled: boolean }) => {
		try {
			const r = await req.post(
				`/projects/features/new?projectid=${activeProject!.project_id}&name=${featureToggle.name}&value=${featureToggle.value}&description=${
					featureToggle.description
				}&enabled=${featureToggle.enabled ? 1 : 0}`,
			);
			if (r && r.status === 200) {
					...
					<ul className="flex">
						<li>
							<Link to={`/projects/${activeProject.project_id}/features`}>
								<button
									type="button"
									onClick={() => onCurrentView('features')}
									className={
										currentView === 'features'
											? 'w-full h-full py-2 px-4 bg-white rounded-t-lg border-l-2 border-t-2 border-r-2 border-gray-200'
						...
						<li>
							<Link to={`/projects/${activeProject.project_id}/settings`}>
								<button
									type="button"
									onClick={() => onCurrentView('settings')}
									className={
						...
						return (
							<div className="overflow-y-scroll pt-4 pr-4" style={{ height: 'calc(100vh - 100px)' }}>
								{showSection('features') && (
									<div>
										...
										{filteredFeaturesByProject.length === 0 && activeProject && (
											<div>No feature toggles for current project!</div>
										)}
								...
								{showSection('settings') && (
									<ProjectSettings currentProject={activeProject!} onProjectDelete={() => onProjectDelete()} />
								)}
								{showSection('api') && (
									<div>
										<div>
				...
				<div className="absolute top-0 bottom-0 right-0 left-0 bg-black bg-opacity-20">
					<div ref={newFeatureWindowRef}>
						<EditFeatureToggle
							...
							onCreateNewFeatureToggle={(featureToggle: { name: string; value: string; description: string; enabled: boolean }) => {
								createNewFeatureToggle(featureToggle);
							}}
						/>
					</div>
				</div>
