...
const Features: React.FC<FeaturesProps> = ({ features, onEditFeature }): React.ReactElement => {
	...
	return (
		<>
			<div className="mb-4">
				<p className="mb-4">Manage feature toggles for current project</p>
				<div className="flex justify-end">
					<div className="relative">
						...
						<svg
							...
							className="absolute right-2 top-2 h-4 w-4 opacity-60"
							fill="none"
							viewBox="0 0 24 24"
							stroke="currentColor"
						>
			...
			<ul className="list-style-none mt-4 p-0">
				{filteredFeatures().map((feature: Feature) => (
					<li
						className="block md:flex justify-between shadow-md border border-solid rounded-md border-gray-200 p-4 mb-4"
						key={feature.project_id + feature.feature_name}
					>
						<div className={!feature.feature_enabled ? 'opacity-50' : ''}>
							...
							<p className="m-0 small text-gray-500">
								Created on:
								{feature.date_created}
							</p>
						</div>
						<div className="flex justify-end align-middle ml-3 mt-3 md:mt-0">
							<Toggle
							...
							/>
							<div className="flex">
								<button type="button" className="mr-4 md:ml-4" onClick={() => onEditFeature(feature)}>
								...
								<button type="button" className="md:ml-4" onClick={() => deleteFeature(feature.project_id, feature.feature_name)}>
									<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
										<path
											strokeLinecap="round"
											strokeLinejoin="round"
